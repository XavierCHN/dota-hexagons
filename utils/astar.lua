...
local binary_heap = require "utils.binary_heap"
...
function startPathing(pathMap, startPos, exitPos)
   ...
   pathMap[startPos].fScore = pathMap[startPos].gScore + pathMap[startPos].hScore
      ...
      for k,v in pairs(testNode.neighbors) do
         if not pathMap[v].closed then
            ...
            if not pathMap[v].open then
               ...
               pathMap[v].fScore = pathMap[v].hScore + tempGScore
...
function newNode(pathLoc, hScore, neighbors, distance)
   ...
   assert(type(neighbors) == "table" and type(next(neighbors)) == "number", "bad arg #1: needs table")
   assert(type(distance) == "table" and type(next(distance)) == "number", "bad arg #1: needs number")
   local n = {
      pathLoc = pathLoc,
      hScore = hScore,
      neighbors = neighbors,
